...
try:
    connection.execute("ALTER TABLE buggies ADD COLUMN total_cost INTEGER DEFAULT 0")
    print("- Added total_cost column to buggies table")
except Exception as e:
    if "duplicate column name" in str(e):
        print("- total_cost column already exists")
    else:
        print(f"- Error adding total_cost column: {e}")
